...
RECIPIENT = BEARS.get_recipient('data/myemail2.txt') #List recipient	
...
class Mailerhi():
	...
	def sent_email(self,server,ehlo,recipient,bcc):
		...
		try:	
			...
			result = server.sendmail(fromaddr,recipient,letter)
			print 'Success'
	...
	# def record_log_and_counter(self,stat_sent):
	...
	def start_sent(self,ehlo,count):
...
def threading_socks(socks,concurrent):
	with futures.ThreadPoolExecutor(max_workers=concurrent) as pool:
		...
		for _ in pool.map(mailer.start_sent,ehlo,range(concurrent)):
			pass		
